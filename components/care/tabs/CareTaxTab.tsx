...
export const CareTaxTab: React.FC<Props> = ({ careCase }) => {
    ...
    // Settings State
    const [taxRateGTGT, setTaxRateGTGT] = useState(5); // %
    const [taxRateTNCN, setTaxRateTNCN] = useState(5); // %
    const [taxThresholdTNCN, setTaxThresholdTNCN] = useState(500000000); // 500tr
    ...
    const financials = useMemo(() => {
        ...
        // Logic: GTGT = Tổng Doanh thu * X%
        // TNCN = (Doanh thu - ngưỡng) * X%
        // X is the unified coefficient (taxRateGTGT is used as X)
        const commonX = taxRateGTGT / 100;
        const taxGTGT = totalRevenue * commonX;
        const taxableRevenueTNCN = Math.max(0, totalRevenue - taxThresholdTNCN);
        const taxTNCN = taxableRevenueTNCN * commonX;

        const totalTax = taxGTGT + taxTNCN;
    ...
    }, [filteredTransactions, taxRateGTGT, taxThresholdTNCN]);

    const totalPaid = payments.reduce((sum, p) => sum + (p.amountTy * 1000000000), 0);
    const taxRemaining = Math.max(0, financials.totalTax - totalPaid);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <Card className="bg-white border-slate-200 shadow-sm">
                    <CardContent className="p-5">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Chi Phí (Tổng)</div>
                        <div className="text-2xl font-black text-rose-600">{formatVND(financials.totalExpense)}</div>
                        <div className="text-xs text-slate-400 font-bold mt-1">Gồm vận hành & sửa chữa</div>
                    </CardContent>
                </Card>
                <Card className="bg-white border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute right-0 top-0 p-4 opacity-5"><Calculator size={64} /></div>
                    <CardContent className="p-5 relative z-10">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Thuế GTGT (Tạm tính)</div>
                        <div className="text-2xl font-black text-amber-600">{formatVND(financials.taxGTGT)}</div>
                        <div className="text-xs text-slate-400 font-bold mt-1">Hệ số: {taxRateGTGT}%</div>
                    </CardContent>
                </Card>
                <Card className="bg-white border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute right-0 top-0 p-4 opacity-5"><Calculator size={64} /></div>
                    <CardContent className="p-5 relative z-10">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Thuế TNCN (Tạm tính)</div>
                        <div className="text-2xl font-black text-indigo-600">{formatVND(financials.taxTNCN)}</div>
                            ...
                            <CardContent className="p-5 space-y-5">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        <div 
                                            onClick={() => {}} 
                                            className="p-4 rounded-xl border-2 border-indigo-600 bg-indigo-50/50 cursor-pointer relative group transition-all"
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-slate-500 mt-0.5">Thuế = Doanh thu * Hệ số</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="p-4 rounded-xl border-2 border-slate-100 bg-white opacity-60 cursor-not-allowed">
                                            <div className="flex items-center gap-3">
                                                <div className="w-5 h-5 rounded-full border-2 border-slate-200 bg-white" />
                                                <div>
                                                    <div className="text-sm font-bold text-slate-400">Theo Lợi Nhuận (DT - CP)</div>
                                                    <div className="text-xs text-slate-400 mt-0.5">Thuế = (DT - CP) * Hệ số</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-2 pt-2">
                                    <label className="text-xs font-bold text-slate-700">Ngưỡng chịu thuế (VNĐ)</label>
                                    <Input
                                        type="text"
                                        value={formatNumber(taxThresholdTNCN)}
                                        onChange={(e: any) => {
                                            const val = e.target.value.replace(/\D/g, '');
                                            setTaxThresholdTNCN(Number(val) || 0);
                                        }}
                                        className="font-bold h-11"
                                    />
                                    <p className="text-[10px] text-slate-400 italic">Tính thuế cho phần doanh thu vượt ngưỡng</p>
                                </div>
                                ...
                                <div className="space-y-2 pt-2">
                                    ...
                                    <div className="relative">
                                        <Input
                                            type="number"
                                            value={taxRateGTGT}
                                            onChange={(e: any) => setTaxRateGTGT(Number(e.target.value))}
                                            className="font-bold pl-8 h-11"
                                        />
                                ...
                                <div className="pt-4">
                                    <Button className="w-full bg-blue-600 hover:bg-blue-700 h-11 text-sm font-bold shadow-lg shadow-blue-100">Lưu cấu hình</Button>
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                                ...
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        <tr className="hover:bg-slate-50/50">
                                            ...
                                            <td className="px-6 py-4 text-right text-xs text-slate-400 italic">Chịu thuế doanh thu</td>
                                        </tr>
                                        <tr className="hover:bg-slate-50/50">
                                            ...
                                            <td className="px-6 py-4 text-right text-xs text-slate-400 italic">Chi phí không tính thuế</td>
                                        </tr>
                                        <tr className="bg-slate-50 font-black text-slate-800">
                                            ...
                                            <td className="px-6 py-4 text-right text-rose-600">{formatVND(financials.totalExpense)}</td>
                                            <td className="px-6 py-4 text-right text-xs text-slate-400 italic whitespace-nowrap overflow-hidden">N/A</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </CardContent>
