...
export const CareTaxTab: React.FC<Props> = ({ careCase }) => {
    ...
    const [taxThreshold, setTaxThreshold] = useState(500000000); // 500tr
    ...
    const financials = useMemo(() => {
        ...
        let taxBase = 0;
        const taxableRevenue = Math.max(0, totalRevenue - taxThreshold);

        if (taxMethod === 'revenue') {
            taxBase = taxableRevenue;
        } else {
            taxBase = Math.max(0, taxableRevenue - taxableExpense);
        }
                            ...
                            <CardContent className="p-5 space-y-5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <Input
                                            type="text"
                                            value={formatNumber(taxThreshold)}
                                            onChange={(e: any) => {
                                                const val = e.target.value.replace(/\D/g, '');
                                                setTaxThreshold(Number(val) || 0);
                                            }}
