...
export const PerformanceReportTab: React.FC<Props> = ({ careCase }) => {
    ...
    const taxThresholdTNCN = 0.5; // 500tr in Billion
    const taxRateGTGT = 0.05; // 5% unified coefficient
    const commonX = taxRateGTGT; 

    const taxGTGT = totals.revenue * commonX;
    const taxTNCN = Math.max(0, totals.revenue - taxThresholdTNCN) * commonX;
    const totalTax = taxGTGT + taxTNCN;

    const netIncome = totals.revenue - totals.cost - totalTax;
    ...
    const handleExportPDF = () => {
        ...
        doc.setFontSize(18);
        doc.text(`Báo cáo tổng hợp ROI - ${careCase.ownerName}`, 14, 22);

        doc.setFontSize(11);
        doc.text(`Mã Care Case: ${careCase.id}`, 14, 30);
        doc.text(`Kỳ báo cáo: Năm ${year}`, 14, 36);

        doc.setFontSize(14);
        doc.text('Tóm tắt tài chính', 14, 46);

        autoTable(doc, {
            startY: 50,
            head: [['Chỉ tiêu', 'Giá trị']],
            body: [
                ['Tổng doanh thu', formatCurrencyTy(totals.revenue)],
                ['Tổng chi phí', formatCurrencyTy(totals.cost)],
                ['Thuế GTGT (tạm tính)', formatCurrencyTy(taxGTGT)],
                ['Thuế TNCN (tạm tính)', formatCurrencyTy(taxTNCN)],
                ['Lợi nhuận ròng', formatCurrencyTy(netIncome)],
                ['Dòng tiền ròng', formatCurrencyTy(netCashflow)],
                ['ROI (Ước tính)', `${roi.toFixed(2)}%`]
            ],
            styles: { font: 'helvetica' } // Helvetica doesn't support full Vi accents natively without a Vibe font, but we go with it for now to avoid complexity of loading fonts.
        });

        const nextY = (doc as any).lastAutoTable.finalY + 10;
        doc.setFontSize(14);
        doc.text('Chi tiết các kỳ', 14, nextY);

        const tableData = aggregatedData.map((row: any) => {
            const net = row.revenue - row.cost - row.tax;
            const rowRoi = (net / mockAssetValue) * 100;
            return [
                row.period,
                formatCurrency(row.revenue, 'cho_thue'),
                formatCurrency(row.cost, 'cho_thue'),
                formatCurrency(row.revenue * 0.05 + Math.max(0, row.revenue - 0.5) * 0.05, 'cho_thue'), // total tax mock
                formatCurrency(net, 'cho_thue'),
                `${rowRoi.toFixed(2)}%`
            ];
        });
        ...
        autoTable(doc, {
            startY: nextY + 4,
            head: [['Kỳ', 'Doanh thu', 'Chi phí', 'Thuế', 'Lợi nhuận', 'ROI']],
            body: tableData,
            styles: { font: 'helvetica' }
        });

        doc.save(`Bao_Cao_${careCase.id}_${year}.pdf`);
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <Card className="border-slate-200 shadow-sm bg-white">
                <CardContent className="p-4 flex flex-col xl:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex items-center gap-4 w-full xl:w-auto justify-end">
                        ...
                        <Button variant="outline" size="sm" className="h-9 gap-2" onClick={handleExportPDF}><Download size={14} /> Xuất báo cáo</Button>
                    </div>
                </CardContent>
            </Card>
            ...
            {loading ? <Skeleton className="h-32 w-full rounded-2xl" /> : (
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                    ...
                    <KPICard title="Thuế GTGT (Tạm)" value={formatCurrencyTy(taxGTGT)} icon={Calculator} color={{ bg: 'bg-amber-50', text: 'text-amber-600' }} sub="5% Doanh thu" />
                    <KPICard title="Thuế TNCN (Tạm)" value={formatCurrencyTy(taxTNCN)} icon={Calculator} color={{ bg: 'bg-indigo-50', text: 'text-indigo-600' }} sub="5% của (DT - 500tr)" />
                    <KPICard title="Lợi nhuận ròng" value={formatCurrencyTy(netIncome)} icon={Activity} color={{ bg: 'bg-emerald-50', text: 'text-emerald-700', textValue: 'text-emerald-700' }} sub="Sau khi trừ tất cả" />
            ...
            <div className="flex flex-wrap gap-4 text-[10px] text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-100">
                ...
                <div>• Lợi nhuận ròng = Doanh thu - Chi phí - Thuế (GTGT + TNCN)</div>
                <div>• Thuế GTGT = 5% x Doanh thu</div>
                <div>• Thuế TNCN = 5% x (Doanh thu - 500tr)</div>
                            ...
                            {loading ? (
                                ...
                                return (
                                    <tr key={idx} className="hover:bg-indigo-50/30 transition-colors group cursor-pointer" onClick={() => setSelectedPeriod(row)}>
                                        ...
                                        <td className="px-6 py-4 text-right font-medium text-amber-600">{formatCurrency(row.revenue * 0.05 + Math.max(0, row.revenue - 0.5) * 0.05, 'cho_thue')}</td>
