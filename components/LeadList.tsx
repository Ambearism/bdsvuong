...
export const LeadList = () => {
    ...
    const handleNavToDeal = (id: string) => {
        // Mock navigation
        toast(`Chuyển hướng đến Giao dịch của Khách ${id}`);
    ...
    const handleDelete = (id: string) => {
        ...
        toast("Đã xóa Khách thành công");
        setSelectedLead(null);
    };

    const getStatusBadge = (status: string) => {
        const map: Record<string, any> = {
            'lead_moi': { label: 'Khách Mới', variant: 'indigo' },
            'dang_cham': { label: 'Đang Chăm', variant: 'blue' },
            'hen_xem_nha': { label: 'Hẹn Xem Nhà', variant: 'amber' },
            'deal_mo': { label: 'Giao dịch Mở', variant: 'warning' },
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {loading ? Array(4).fill(0).map((_, i) => <Skeleton key={i} className="h-28 w-full rounded-2xl" />) : (
                    <>
                        <Card className="hover:-translate-y-1 transition-transform border-none shadow-sm ring-1 ring-slate-200">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div>
                                    <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">Tổng Lead</p>
                                    <p className="text-3xl font-black text-slate-800 mt-2">{formatNumber(data?.kpi.total || 0)}</p>
                                </div>
                                <div className="p-4 bg-indigo-50/50 text-indigo-600 rounded-2xl"><Users size={24} strokeWidth={2} /></div>
                            </CardContent>
                        </Card>
                        <Card className="hover:-translate-y-1 transition-transform border-none shadow-sm ring-1 ring-slate-200">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div>
                                    <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">Khách Mới Tuần Này</p>
                                    <div className="flex items-baseline gap-2 mt-2">
                                        <p className="text-3xl font-black text-slate-800">{formatNumber(data?.kpi.newThisWeek || 0)}</p>
                                        <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-md">+12%</span>
                                    </div>
                                </div>
                                <div className="p-4 bg-emerald-50/50 text-emerald-600 rounded-2xl"><UserPlus size={24} strokeWidth={2} /></div>
                            </CardContent>
                        </Card>
                        <Card className="hover:-translate-y-1 transition-transform border-none shadow-sm ring-1 ring-slate-200">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div>
                                    <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">Khách → Giao dịch</p>
                                    <p className="text-3xl font-black text-slate-800 mt-2">{formatNumber(data?.kpi.leadToDeal || 0)}</p>
                                </div>
                                <div className="p-4 bg-amber-50/50 text-amber-600 rounded-2xl"><Briefcase size={24} strokeWidth={2} /></div>
                            </CardContent>
                        </Card>
                        <Card className="hover:-translate-y-1 transition-transform border-none shadow-sm ring-1 ring-slate-200">
                            <CardContent className="p-6 flex items-center justify-between">
                                <div>
                                    <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">Tỷ Lệ Chuyển Đổi</p>
                                    <p className="text-3xl font-black text-slate-800 mt-2">{data?.kpi.conversionRate}%</p>
                                </div>
                                <div className="p-4 bg-rose-50/50 text-rose-600 rounded-2xl"><Users size={24} strokeWidth={2} /></div>
                            </CardContent>
                        </Card>
                    </>
                )}
