...
export const CardTitle = ({ className, children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
  <h3 className={cn("font-bold text-base text-slate-800", className)} {...props}>
    {children}
  </h3>
);
...
export const Select = ({ label, value, onChange, options, className, wrapperClassName, placeholder, error, showClear, disabled }: any) => {
    ...
    return (
        <div className={cn("space-y-1.5 w-full", wrapperClassName)} ref={ref}>
...
export const Badge = ({ children, variant = 'neutral', className }: any) => {
    ...
    return (
        <span className={cn("inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wide", variants[variant], className)}>
            {children}
        </span>
    );
};
...
export const Tabs = ({ activeTab, onChange, tabs, className }: { activeTab: string, onChange: (id: string) => void, tabs: {id: string, label: string}[], className?: string }) => (
    <div className={cn("flex p-1 bg-slate-100 rounded-lg border border-slate-200", className)}>
        {tabs.map(tab => (
            <button
                key={tab.id}
                ...
                className={cn(
                    ...
                    activeTab === tab.id ? "bg-white text-indigo-700 shadow-sm ring-1 ring-slate-200" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50"
                )}
            >
                {tab.label}
            </button>
