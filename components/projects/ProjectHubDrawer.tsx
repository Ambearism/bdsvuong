...
import { Building2, Search, Filter, Upload, Image as ImageIcon, ExternalLink, Edit2, Check, X } from 'lucide-react';
                    ...
                    <Card className="border-slate-200 shadow-sm bg-white shrink-0">
                        <CardContent className="p-4 md:p-5">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
                                <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-4 gap-3 items-center">
                                    <div className="relative w-full sm:col-span-1 xl:col-span-2">
                                        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                        <Input
                                            className="pl-10 h-10 border-slate-200 bg-slate-50 focus:bg-white text-sm w-full"
                                    ...
                                    <div className="flex items-center gap-2 w-full">
                                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden xl:block">Lọc:</div>
                                        <Select
                                ...
                                <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-0 border-slate-100">
                                    <Button className="gap-2 bg-indigo-600 hover:bg-indigo-700 shadow-md h-10 px-4 text-white text-sm flex-1 sm:flex-none" onClick={() => setIsUploadOpen(true)}>
                                        <Upload size={16} /> Upload Ảnh
                                    </Button>
                                    <TooltipWrapper content="Xem toàn màn hình">
                                        <Button variant="outline" size="icon" className="h-10 w-10 border-slate-200 text-slate-500 hover:bg-slate-100 shrink-0" onClick={() => { onClose(); window.dispatchEvent(new CustomEvent('routeChange', { detail: { route: 'project_hub', id: projectId } })); }}>
                            ...
                            <table className="w-full text-left border-collapse min-w-[700px]">
                                <thead className="bg-slate-50 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3">Tên HH / Căn Số</th>
                                        <th className="px-4 py-3">Phân Lô/Tòa</th>
                                    ...
                                    {loading ? (
                                        ...
                                        <tr key={item.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={(e) => handleViewPropertyHub(item.id, e)}>
                                            <td className="px-4 py-3">
                                                <div className="w-12 h-12 rounded-xl overflow-hidden shadow-sm border border-slate-200 group-hover:shadow-md transition-shadow">
                                            ...
                                            <td className="px-4 py-3">
                                                {editingId === item.id ? (
                                                    <div className="space-y-1">
                                                        <Input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })} className="h-7 text-xs w-[140px]" onClick={e => e.stopPropagation()} />
                                                        <Input value={editForm.unitNo} onChange={e => setEditForm({ ...editForm, unitNo: e.target.value })} className="h-7 text-xs w-[100px]" onClick={e => e.stopPropagation()} placeholder="Căn số" />
                                                    </div>
                                                ) : (
                                                    <>
                                                        <div className="font-black text-slate-900">{item.name}</div>
                                                        <div className="text-xs font-bold text-slate-500 mt-0.5">Căn: <span className="text-slate-700">{item.unitNo}</span></div>
                                                    </>
                                                )}
                                            </td>
                                            <td className="px-4 py-3">
