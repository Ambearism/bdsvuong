...
const KPI_DATA = [
    ...
    { label: "Lead mới → Hẹn Xem", value: "36,8%", icon: UserPlus, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Deal Mở → Đàm Phán", value: "21,3%", icon: TrendingUp, color: "text-amber-600", bg: "bg-amber-50" },
    ...
    { label: "Lead → Thất Bại", value: "1,1%", icon: XCircle, color: "text-slate-500", bg: "bg-slate-100" },
    { label: "Deal → Thất Bại", value: "0,7%", icon: AlertTriangle, color: "text-rose-500", bg: "bg-rose-50" },
];
            ...
            <Card className="mb-6 border border-slate-200 shadow-sm shrink-0">
                <CardContent className="p-4 flex flex-col xl:flex-row gap-4 items-start xl:items-center">
                    ...
                    <div className="flex gap-2 overflow-x-auto w-full no-scrollbar pb-1">
                        {['Nguồn Lead', 'Trạng Thái', 'Sale Phụ Trách', 'Nhu Cầu', 'Loại BĐS', 'Khu Vực', 'Ngân Sách'].map((label, i) => (
                             <Select key={i} className="min-w-[140px] text-xs" label={label} placeholder="Tất cả" options={[]} value="" onChange={()=>{}} />
                        ))}
                    </div>
                </CardContent>
            </Card>
