...
export const LeaseTransactionsTab: React.FC<Props> = ({ lease, transactions, schedule, allocations }) => {
  ...
  const handleAutoAllocate = () => {
    ...
    let balance = txRemainingAmount;
    const newAlloc: Record<string, number> = {};
    for (const period of outstandingPeriods) {
        if (balance <= 0) break;
        const needed = period.amountTy - period.amountPaidTy;
        const canPay = Math.min(balance, needed);
        newAlloc[period.id] = canPay;
        balance -= canPay;
    }
    setManualAllocations(newAlloc);
    toast("Đã tính toán phân bổ tự động (FIFO)");
  };
                ...
                <>
                    <Card className="border-indigo-200 bg-white shadow-md overflow-hidden ring-1 ring-indigo-50">
                        <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/50 border-b border-slate-100">
                            ...
                            <Button size="sm" variant="outline" className="bg-white border-indigo-200 text-indigo-600 h-10 px-4 font-bold" onClick={handleAutoAllocate}>
                                <Zap size={14} className="mr-2"/> FIFO
                            </Button>
                        </div>
