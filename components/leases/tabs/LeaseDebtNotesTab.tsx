...
export const LeaseDebtNotesTab: React.FC<Props> = ({ lease, notes: initialNotes }) => {
  ...
  const handleAddNote = () => {
    ...
    const note: DebtNote = {
      id: Date.now().toString(), leaseId: lease.id, content: newNote, tag: 'other', isInternal: true,
          ...
          <CardContent className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-2 text-xs font-bold">
                ...
                <select
                  className="bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-slate-700"
                  value={assignee}
                  onChange={(e) => setAssignee(e.target.value)}
                >
                  <option value="Trịnh CSKH">Trịnh CSKH</option>
                  <option value="Lê Kế Toán">Lê Kế Toán</option>
          ...
          <div className="space-y-6 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
            {notes.map((note) => (
              <div key={note.id} className="relative pl-14">
                ...
                <div className={cn(
                  "bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all"
                )}>
