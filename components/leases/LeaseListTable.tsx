...
import { Edit, Eye, Trash2, ArrowRightLeft, PowerOff, AlertCircle, Clock, CalendarPlus, AlertTriangle, FileText, CheckCircle2, MoreHorizontal, Plus, ExternalLink } from 'lucide-react';
import { ViewContractModal } from '../common/modals/ViewContractModal';
...
export const LeaseListTable: React.FC<Props> = ({ data, loading, onViewHub, onRenew, onTransfer, onAddCashflow, onAddDebtNote }) => {
  const [viewContractTarget, setViewContractTarget] = useState<Lease | null>(null);
    ...
    <div className="overflow-x-auto custom-scrollbar">
              ...
              <td className="sticky right-0 bg-white group-hover:bg-slate-50 px-2 py-4 shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.05)] text-center z-20" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-center gap-1">
                  ...
                  <TooltipWrapper content="Xem Chi Tiết">
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:bg-slate-100" onClick={(e) => { e.stopPropagation(); setViewContractTarget(lease); }}>
                      <Eye size={14} />
                    </Button>
                  </TooltipWrapper>
                </div>
      ...
      {viewContractTarget && (
        <ViewContractModal
          ...
          title={`Hợp đồng ${viewContractTarget.code}`}
          subtitle={`${viewContractTarget.tenantName} - ${viewContractTarget.propertyName}`}
          badgeText={STATUS_MAP[viewContractTarget.status].label}
          headerColor="bg-indigo-600"
          documents={[
            {
              id: 'doc-1',
              name: 'Hopdongthuenha_Chinhthuc.pdf',
              url: '#',
              sizeKb: 1540,
              uploadedAt: new Date(viewContractTarget.createdAt).toLocaleDateString('vi-VN')
            },
            {
              id: 'doc-2',
              name: 'PhuLuc_HopDong.pdf',
              url: '#',
              sizeKb: 450,
              uploadedAt: new Date(viewContractTarget.updatedAt).toLocaleDateString('vi-VN')
            }
          ]}
        />
      )}
