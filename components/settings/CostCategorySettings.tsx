...
export const CostCategorySettings = () => {
    ...
    const handleAddGroup = () => {
        setEditingGroup({
            id: '', code: '', name: '', description: '', displayOrder: groups.length + 1, isActive: true, isDefaultSeeded: false
        });
    ...
    const handleAddItem = (groupId: string, e?: React.MouseEvent) => {
        ...
        setEditingItem({
            id: '', groupId, code: '', name: '', description: '', examples: '',
            tenantRelatedFlag: groupId === 'GRP_TENANT', // Intelligent default
            isTaxDeductible: false, requiresAttachment: false, isActive: true, 
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-lg font-bold text-slate-800">Cấu hình Danh mục Chi phí</h2>
                    <p className="text-sm text-slate-500">Phân loại chi phí vận hành để chuẩn hóa báo cáo ROI & Thuế</p>
                </div>
            ...
            {/* Group Modal */}
            <Modal isOpen={groupModalOpen} onClose={() => setGroupModalOpen(false)} title={editingGroup?.id ? "Chỉnh sửa Nhóm" : "Thêm Nhóm Chi Phí"} size="md">
                <div className="p-6 space-y-4">
                    ...
                    <Textarea label="Mô tả" value={editingGroup?.description} onChange={e => setEditingGroup({...editingGroup!, description: e.target.value})} />
            ...
            {/* Item Modal */}
            <Modal isOpen={itemModalOpen} onClose={() => setItemModalOpen(false)} title={editingItem?.id ? "Chỉnh sửa Mục Chi Phí" : "Thêm Mục Chi Phí"} size="lg">
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                            <div>
                                <div className="text-sm font-bold text-slate-700">Được khấu trừ thuế</div>
                                <div className="text-[10px] text-slate-500">Đánh dấu chi phí hợp lý</div>
                            </div>
