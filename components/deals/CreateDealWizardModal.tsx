...
const STEPS = [
    { id: 1, label: "Thông tin Deals & Khách hàng" },
    { id: 2, label: "BĐS & Điều kiện giao dịch" },
    { id: 3, label: "Nguồn Deals & phân bổ" },
    ...
    const validateStep = async (currentStep: number): Promise<boolean> => {
        ...
        if (currentStep === 1) {
            if (formData.linkMode === 'from_lead' && !formData.leadId) {
                newErrors.leadId = "Vui lòng chọn Lead để liên kết";
            }
            ...
            if (formData.linkMode === 'direct' && formData.phone.includes("999")) {
                const confirmLink = window.confirm("SĐT này trùng với Lead #00123. Bạn có muốn liên kết với Lead đó thay vì tạo mới?");
                if (confirmLink) {
        ...
        if (currentStep === 3) {
            if (!formData.dealSource) newErrors.dealSource = "Chọn nguồn Deals";
            ...
            if (formData.dealStage === 'hoan_tat' && !window.confirm("Đánh dấu deal hoàn tất? Hành động này sẽ cập nhật KPI.")) return false;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-[1000px] h-[95vh] md:h-auto md:max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Tạo Deals Mới</h2>
                            ...
                            {step === 1 && (
                                <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="bg-indigo-50/50 border border-indigo-100 p-4 rounded-xl">
                                        <label className="text-sm font-semibold text-slate-700 mb-3 block">Nguồn tạo Deals:</label>
                                        <div className="flex gap-6 mb-4">
                                            <label className="flex items-center gap-2 cursor-pointer">
                                                ...
                                                <span className="text-sm font-medium text-slate-700">Tạo từ Lead có sẵn</span>
                                            </label>
                                            <label className="flex items-center gap-2 cursor-pointer">
                                                ...
                                                <span className="text-sm font-medium text-slate-700">Tạo Deals trực tiếp</span>
                                            </label>
                                        </div>
                                        ...
                                        {formData.linkMode === 'from_lead' && (
                                            <div className="animate-in fade-in duration-200">
                                                <Select 
                                                    label="Chọn Lead (Tìm kiếm theo Tên/SĐT)" 
                            ...
                            {step === 2 && (
                                <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2 border-t border-slate-100">
                                        <Input label="Giá trị Deals dự kiến (Tỷ)" type="number" step="0.1" value={formData.dealValueTy} onChange={e => handleChange('dealValueTy', e.target.value)} />
                            ...
                            {step === 3 && (
                                <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <Select label="Nguồn Deals" required value={formData.dealSource} onChange={v => handleChange('dealSource', v)} error={errors.dealSource} options={MOCK_DEAL_SOURCES} />
                                    ...
                                    {formData.dealStage === 'huy' && (
                                        <div className="p-4 bg-rose-50 text-rose-800 rounded-xl border border-rose-100 flex items-start gap-3">
                                            ...
                                            <span className="text-sm">Xác nhận chuyển trạng thái <b>Hủy/Thất Bại</b>. Deal sẽ bị đóng lại.</span>
                                        </div>
                                    )}
                                </div>
                            )}
