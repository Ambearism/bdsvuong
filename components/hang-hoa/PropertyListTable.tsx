          ...
          {data.map((item) => (
            <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
              ...
              <td className="px-4 py-3">
                <a href={`/hang-hoa/${item.id}/hub?from=list`} onClick={(e) => handleHubClick(e, item.id)} className="font-semibold text-indigo-700 hover:underline line-clamp-1 cursor-pointer whitespace-nowrap">
                  {item.name}
                </a>
              </td>
              ...
              <td className="px-4 py-3 text-right text-xs text-slate-500 whitespace-nowrap font-medium min-w-[150px]">
                <span className="inline-flex items-center gap-2">
                  {formatDateTimeVi(item.createdAt).split(' ')[0]}
                  <span className="text-[10px] text-slate-300">{formatDateTimeVi(item.createdAt).split(' ')[1]}</span>
                </span>
              </td>
              <td className="px-4 py-3 text-right text-xs text-slate-500 whitespace-nowrap font-medium min-w-[100px]">
