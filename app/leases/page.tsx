                ...
                <div className="flex flex-col xl:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                        ...
                        <div className="flex items-center gap-2 mr-4">
                            <button onClick={() => setFilters(p => ({...p, paymentStatus: 'overdue'}))} className={cn("text-xs font-bold px-3 py-1.5 rounded-full border transition-all", filters.paymentStatus === 'overdue' ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-white text-slate-500 border-slate-200 hover:border-rose-200")}>
                                Overdue
                            </button>
                            <button onClick={() => setFilters(p => ({...p, status: 'expiring'}))} className={cn("text-xs font-bold px-3 py-1.5 rounded-full border transition-all", filters.status === 'expiring' ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-white text-slate-500 border-slate-200 hover:border-amber-200")}>
                                Sắp hết hạn
                            </button>
                        </div>

                        <Button variant="ghost" size="sm" onClick={() => setShowAdvanced(!showAdvanced)} className={cn("text-slate-500 hover:text-indigo-600", showAdvanced && "bg-indigo-50 text-indigo-600")}>
                ...
                {showAdvanced && (
                    <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-4 pt-4 border-t border-slate-100 animate-in slide-in-from-top-2">
                        ...
                        <div className="col-span-2 flex items-end justify-end">
                             <Button variant="ghost" size="sm" onClick={() => setFilters(DEFAULT_FILTERS)} className="text-rose-500 hover:bg-rose-50">
