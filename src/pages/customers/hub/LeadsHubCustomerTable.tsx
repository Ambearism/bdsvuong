...
const LeadsHubCustomerTable = ({ enumData }: { enumData: ApiResponse<EnumData> | undefined }) => {
    const { hasPermission } = usePermission()
    const navigate = useNavigate()
    const { customerData, leadsData, leadsAndDealsPagination, handleLeadsAndDealsPagination, refetchLeadsAndDeals } =
    ...
    const leadsColumns: ColumnsType<OpportunityItem> = [
        ...
        },
        {
            ...
            render: (stage: OpportunityStageType, record) => {
                const canUpdate = hasPermission(RESOURCE_TYPE.LEAD, ACTION.UPDATE)
                return (
                    <Select
                        value={stage}
                        className="w-full"
    ...
    return (
        <Card
            title={
                <Flex justify="space-between" align="center">
                    <Space align="center" size={4}>
                        <UserOutlined />
                        <span>Leads Liên Quan ({leads.length})</span>
                    </Space>
            ...
            <Table<OpportunityItem>
                ...
                locale={{ emptyText: <Empty description="Chưa có Leads liên kết" /> }}
            />
        </Card>
    )
}

export default LeadsHubCustomerTable
