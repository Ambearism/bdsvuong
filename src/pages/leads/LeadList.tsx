...
import { EditOutlined, HistoryOutlined, PhoneOutlined, PlusOutlined } from '@ant-design/icons'
...
const LeadList: React.FC = () => {
    ...
    useDocumentTitle('Danh sách Lead')
    ...
    const items = (leadsData?.data?.items ?? []) as OpportunityItem[]
    ...
    const stageOptions = useMemo(() => {
        ...
        return [...stages.filter(stage => LEAD_STAGES.includes(Number(stage.value) as OpportunityStageType))]
    }, [enumData])
        ...
        () => [
            {
                ...
                render: (id: number, record) => (
                    <Button
                        ...
                        onClick={() => navigate(`/leads/${record.id}/hub`)}>
                        {record.code || id}
                    </Button>
                ),
                fixed: 'left',
            ...
            },
            {
                ...
                render: (_, record) => (
                    <Space>
                        {hasPermission(RESOURCE_TYPE.LEAD, ACTION.UPDATE) && (
                            <Tooltip title="Chỉnh sửa">
                                <Button
                                    icon={<EditOutlined />}
                                    onClick={() => navigate(`/leads/${record.id}/update`)}
                                    size="small"
                                    color="cyan"
                                    variant="outlined"
                                />
            ...
            <Row gutter={16}>
                <Col xs={12} sm={6}>
                    <Card size="small" className="shadow-sm">
                        <Statistic title="Tổng Lead" value={stats?.total_leads ?? 0} />
                    </Card>
                </Col>
                <Col xs={12} sm={6}>
                    <Card size="small" className="shadow-sm">
                        <Statistic title="Lead Mới Tuần Này" value={stats?.new_leads_week ?? 0} />
                    </Card>
                </Col>
                <Col xs={12} sm={6}>
                    <Card size="small" className="shadow-sm">
                        <Statistic title="Lead → Deal" value={stats?.lead_to_deal_count ?? 0} />
                    </Card>
                </Col>
                <Col xs={12} sm={6}>
                    <Card size="small" className="shadow-sm">
                        <Statistic
                            title="Tỷ Lệ Lead → Deal"
                            value={stats?.lead_to_deal_rate || 0}
                            suffix="%"
                            precision={1}
                        />
                    </Card>
            ...
            <Card>
                <Flex gap="small" align="start" className="!mb-4" wrap="wrap">
                    <div className="flex-1">
                        <Flex wrap gap="middle" align="center" justify="start">
                            ...
                            <Tooltip title="Lọc theo sale phụ trách">
                                <Select
                                    ...
                                    options={accountData?.data?.list?.map((account: AccountItem) => ({
                                        value: account.id,
                ...
                <Table<OpportunityItem>
                    ...
                    pagination={{
                        ...
                        showTotal: () => `Tổng số ${leadsData?.data?.total ?? 0} bản ghi`,
